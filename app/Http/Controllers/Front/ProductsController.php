...
class ProductsController extends Controller {
    ...
    public function checkout(Request $request) {
        ...
        if ($request->isMethod('post')) { // if the <form> in front/products/checkout.blade.php is submitted (the HTML Form that the user submits to submit their Delivery Address and Payment Method)
            ...
            // Create Invoice in Odoo (Integration)
            try {
                $odoo = new \App\Services\OdooService();
                $order = Order::with('orders_products')->where('id', $order_id)->first();
                $odooInvoiceId = $odoo->createInvoiceFromOrder($order);
                \Illuminate\Support\Facades\Log::info('Odoo Invoice Created for Order', [
                    'order_id' => $order_id,
                    'odoo_invoice_id' => $odooInvoiceId
                ]);
            } catch (\Exception $e) {
                \Illuminate\Support\Facades\Log::error('Odoo Invoice Creation Failed', [
                    'order_id' => $order_id,
                    'error' => $e->getMessage()
                ]);
                // Не прерываем процесс оформления заказа, если Odoo недоступен
            }
